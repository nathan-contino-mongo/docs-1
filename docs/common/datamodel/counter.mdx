...
flightIds.forEach {
  try! collection.upsert([
      "_id": $0,
      "name": "Flight ABC123",
      "orderedCount": Counter(),
      "totalAvailable": 300
  ], writeStrategy: .insertDefaultIfAbsent)
}
```

When we call `Counter()`, the counter `orderedCount` is initialized to 0.
...
.onPlusDidClick {
    seatDocument.update { doc in
        doc?["orderedCount"].increment(amount: 1)
    }
}
```
...
ditto.store["drinks"].findByID(docID).update { mutableDoc in
    mutableDoc?["usedCount"].increment(amount: 11)
}

print(ditto.store["drinks"].findByID(docID)) // {usedCount: 11}

ditto.store["drinks"].findByID(docID).update { mutableDoc in
    mutableDoc?["usedCount"].increment(amount: 10)
}
...
ditto.store["drinks"].findByID(docID).update { mutableDoc in
    mutableDoc?["usedCount"].increment(amount: 1)
}
