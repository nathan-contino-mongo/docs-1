...
* Use `DittoRegister` to create Registers 
...
```swift
coll.findByID(docID).update(mutDoc => {
  // This is not ok. You should modify a register at an index or path
  // otherwise you will lose any concurrent updates and the type will change
...
Arrays and start using Registers instead as soon as possible.
...
In your schema validation, distributed applications typically only add new fields and
never remove or modify existing fields in production. Despite these validation
code paths, bugs happen. It could be possible to have a version of your
application that changed the type of a field that an older application version
is using. This could cause a crash in production if an application version is
expecting a different shape to your data.  
